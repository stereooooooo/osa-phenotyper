...
function validateIntakeData(body) {
  ...
  // ── Cardiovascular and echocardiogram history (required) ─
  const cvdAnswers = new Set(['yes', 'no', 'unsure']);
  const echoAnswers = new Set(['yes', 'no', 'unsure']);
  const resultAnswers = new Set(['exact', 'normal', 'reduced', 'unknown']);
  if (typeof body.cardiovascularHistory !== 'object' || body.cardiovascularHistory === null || Array.isArray(body.cardiovascularHistory)) {
    errors.push('cardiovascularHistory');
  } else {
    const source = body.cardiovascularHistory;
    ...
    if (!cvdAnswers.has(source.historyAnswer) || source.hasCvd !== (source.historyAnswer === 'yes')) {
      ...
      if (!echoAnswers.has(source.echoHistory)) {
        ...
        cardiovascularHistory.echoHistory = source.echoHistory;
        if (source.echoHistory === 'yes') {
          if (!resultAnswers.has(source.resultKnowledge)) {
            errors.push('cardiovascularHistory.resultKnowledge');
          } else {
            cardiovascularHistory.resultKnowledge = source.resultKnowledge;
            if (source.resultKnowledge === 'exact') {
              if (!isNumInRange(source.lvef, 5, 90)) {
                errors.push('cardiovascularHistory.lvef');
              } else {
                cardiovascularHistory.lvef = source.lvef;
              }
            }
          }
        }
...
async function handlePostIntake(event, rawToken, rawBody) {
  ...
  const lvefFollowupNeeded = needsLvefFollowup(mergeResult.mergedFormData, mergeResult.pendingOverrides);
  mergeResult.mergedFormData.lvefFollowupNeeded = lvefFollowupNeeded ? 'on' : '';
  ...
  let patientUpdateExpression =
    ...
    '#version = :nextVersion, #intakeAppliedFieldCount = :appliedCount, #intakePendingFieldCount = :pendingCount, #lvefFollowupNeeded = :lvefFollowupNeeded';
