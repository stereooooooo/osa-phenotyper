...
| 243 | Questionnaire-to-staff handoff | The real patient questionnaire opens on the same localhost origin with synthetic answers already filled; submission returns through a browser-only same-origin channel even without `window.opener`, visibly adds CPAP history and treatment preferences, preserves the documented sleep-study fields for Staff Review, and changes no questionnaire content. |
| 244 | Documented WatchPAT demo fixture | The main meeting chart begins with internally consistent moderate WatchPAT results already documented, so Staff Review can verify the study without an artificial import step. The fixture does not activate positional OSA or elevated hypoxic-burden signals that would distract from the Inspire pathway. |
