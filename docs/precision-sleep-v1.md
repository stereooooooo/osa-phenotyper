...
**Implementation status (2026-07-19):** Initial clinician-only workflow started. It supports structured manual entry, a verification-gated ResMed AirView-oriented PDF parser, manufacturer-aware leak context, symptom-download discordance, central-event safety context, and editable clinician disposition. The parser has been checked against an AirView 4.51 compliance plus therapy report and captures configured APAP range, usage, pressure percentile, source-report leak threshold, leak, residual event components, and periodic breathing. Expansion to other vendors and patient education remains gated on safely redacted report examples and clinician validation.
