...
| Clinical logic reviewed through | `ER-2026-08-13-COMISA-HYPOXEMIA-WEIGHT`; targeted documentation review completed with no runtime change; source-verified wording changes remain pending clinician signoff; see [`test-matrix-results.md`](test-matrix-results.md) for current regression results |
...
| TX-01 | COMISA sequencing and safety | ISI at least 15 plus AHI at least 5 flags a local operational screening pattern, not an insomnia diagnosis. CBT-I is offered early; PAP may begin concurrently or sequentially based on severity, substantial nocturnal hypoxemia, sleepiness, safety-sensitive duties, access, and preference. High baseline ESS triggers early sleepiness and safety monitoring during bedtime restriction, not a CBT-I contraindication. PAP mode and settings remain individualized. | E2/E5 | Evidence informed with a local screening boundary | `citations.md`, COMISA | The strongest treatment trials generally required confirmed insomnia and AHI at least 15, so application to a mild-OSA numerical screen is indirect. Adherence benefit varies across trials; no validated rule selects sequence, PAP mode, pressure range, or CBT-I pacing for an individual. | Clinician confirmation of the screen, ISI, PAP uptake and adherence, sleepiness trajectory, safety events, patient-reported benefit, sequence chosen, and clinician override rate |
